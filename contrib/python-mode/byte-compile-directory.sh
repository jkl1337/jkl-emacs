...
$EMACS -Q --batch --eval "(message (emacs-version))" --eval "(when (featurep 'python-mode)(unload-feature 'python-mode t))" --eval "(add-to-list 'load-path \"$PDIR/\")" -load "pymacs" -load "$PDIR/$PYTHONMODE" --eval '(byte-recompile-directory default-directory 0 t)'
